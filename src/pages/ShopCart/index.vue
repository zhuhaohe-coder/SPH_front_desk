        ...
        <ul class="cart-list" v-for="shop in shopCartList" :key="shop.id">
          ...
          <li class="cart-list-con5">
            <a
              ...
              class="mins"
              @click="skuNumUpdate(shop.skuId, -1, shop.skuNum)"
              >-</a
            >
            <input
              autocomplete="off"
              ...
              @change="
                skuNumUpdate(
                  shop.skuId,
                  $event.target.value - shop.skuNum,
                  shop.skuNum
                )
              "
            />
...
export default {
  ...
  methods: {
    ...
    // skuNum: 变化的数量 numNow当前的数量
    async skuNumUpdate(skuId, skuNum, numNow) {
      if (skuNum < 0 && numNow < 1) {
        return;
      }
      try {
  ...
  computed: {
    ...
    // 总数
    totalNum() {
      return this.shopCartList.reduce((pre, cur) => pre + cur.skuNum, 0);
    },
    // 全选
    isCheckedAll() {
